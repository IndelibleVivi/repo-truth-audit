...
- 用完整记录验收堵住 query-echo 漏检；检查自然局部采纳、内部实现委托，以及功能齐全却增加
  未经接受的日常劳动的流程，并加入后续授权对照。
- 并列说明运行事实与产品意图两条适用入口，覆盖新建小仓库，保留普通 bugfix/review 的排除边界。
- 正式发布 v0.3.0，同步双语发布说明与稳定安装引用；公开 tag 临时安装与已验收九文件 payload 一致。
