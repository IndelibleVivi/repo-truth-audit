...
Repo Truth Audit 为一个明确的所有者决策恢复仓库的当前 operational truth，或对照已接受
产品意图与实际行为。意图对照同样适用于新建、小型、结构整洁的仓库。
当用户明确要求有限结构变更或产品意图收敛时，同一个产品可以从诊断继续到有限计划、获授权的
...
自然回应不需要正式采纳标签，应结合局部上下文解释，包括局部批准、试用范围与内部实现委托。
已接受结果和对内部选择的授权可以支持接线、文件布局等决定，无需逐项补批准；这些依据也不
允许改变结果或增加用户义务。
...
对流程约束，比较谁必须做什么、何时及多频繁做、通过什么 gate 才能继续。可选观察界面
变成必经流程时，即使功能齐全，也可能增加未经接受的日常劳动；后来的 owner 决策可以在
对应范围内合理改变这些义务。
...
意图对照先给还原出的产品形态、正常使用要求与异常时才需要的检查、权威与覆盖限制，再给实质差距和被排除的误报；区分当前
承诺与未来可能性。
...
Candidate 返修覆盖自然局部采纳、内部实现委托、功能齐全但流程增负，以及后续明确授权的
对照。Search 修复 witness 必须验证完整匹配记录，不能只检查回显的 query。通过给定
candidate catalog 做的方法选择检查，不等于宿主自动发现验收。
