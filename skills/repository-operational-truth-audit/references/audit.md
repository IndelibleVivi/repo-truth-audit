# Read-only operational-truth method

Use this method for a standalone audit or the diagnostic phase of an authorized
operation. Its findings and evidence rules are shared; in operate mode return
to operation.md after diagnosis instead of treating this phase as final delivery.
Audit-mode analysis remains read-only.
...
to manufacture proof. Inspect unknown target-controlled programs before executing
them. Disposable copies and Git worktrees do not contain network, credentials,
or external effects; require suitable host-enforced controls for untrusted
execution. Without them, retain the decisive execution question as unobserved.
...
or unbounded loop-until-clean workflow. Explicitly authorized restructuring
continues under operation.md within this same product.
